...
Fecha        Monto          Desde -> Hacia
-----------------------------------------
2026-02-26  1000 USD  Bussa -> Banco
2026-02-26  1000 EUR  Banco -> Bussa
...
Fecha        Monto          Desde -> Hacia
-----------------------------------------
2026-02-26  1000 USD  Banco -> Bussa

Fin> :reporte reporte.html
...
\texttt{:quote [<nombre> <val>]} & Ver/definir cotización del oráculo \\
\texttt{:partes [<yo> <contra>]} & Ver/cambiar las partes del contrato \\
\texttt{:setfecha <YYYY-MM-DD>} & Cambiar fecha de evaluación desde el REPL \\
\texttt{:wallet [<parte>]} & Ver billetera(s) \\
\texttt{:pending} & Ver contratos pendientes de firma \\
\texttt{:historial} & Ver historial de ejecuciones \\
\texttt{:reporte [<archivo.html>]} & Exportar reporte HTML completo \\
\texttt{:ast-svg <contrato>} \textbf{'>'} \texttt{<archivo>} & Exportar AST del contrato a SVG \\
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Exportar.hs.} Genera reportes HTML autocontenidos con gráficos SVG embebidos (incluyendo barras de cashflows, diagramas de árbol SVG del AST, y gráficos de evolución temporal de billeteras).
\end{itemize}
...
El comando \texttt{:ast-svg} genera un diagrama SVG del árbol de sintaxis abstracta de un contrato, utilizando un algoritmo de layout que calcula posiciones centradas para cada nodo. Los nodos se colorean según el tipo de combinador (azul para \texttt{and}, rojo para \texttt{give}, verde para \texttt{one}, etc.).

\subsubsection{Gráfico de barras SVG}

Cada entrada del historial en el reporte HTML incluye un gráfico SVG completo con tres secciones: tabla de cashflows, gráfico de barras horizontales (desde la perspectiva de una parte) con barras verdes para ingresos y rojas para egresos, y resumen neto por moneda.
...
Todo comando del REPL puede abreviarse a su primer carácter (por ejemplo, \texttt{:q} equivale a \texttt{:quit}, \texttt{:r} a \texttt{:reporte}, etc.).
